...
const HrJobPost = ({ }) => {
    ...
    const getjobDetails = async () => {
        try {
            const res = await HttpClient.requestData("view-job-byId/" + location?.state?.data?.jobId, "POST", {})
            const newData = res?.data?.[0];
            setHiringManagerID(newData.hiringManager)
            console.log(newData, "jobdetailsbyid")
            setAddJob(prev => ({
                ...prev,
                jobTitle: newData?.jobTitle,
                jobLocation: newData?.jobLocation,
                level: newData?.level,
                isNewPosition: newData?.isNewPosition,
                isTempPosition: newData?.isTempPosition,
                hiringReason: newData?.hiringReason,
                positionDescription: newData?.positionDescription,
                interviewerEmail: newData?.interviewerEmail,
                altEmail: newData?.altEmail,
                openings: parseInt(newData?.openings),
                keySkills: newData?.keySkills?.map((item) => item),
                keyResponsibilities: newData?.responsibilities,
                qualification: newData?.qualification,
                addQualification: newData?.addQualification,
                experience_high: newData?.experience_high,
                experience_low: newData?.experience_low,
                Competencies: newData?.Competencies,
                inclusion: newData?.inclusion,
                salary: newData?.salary,
                salaryDate: newData?.salaryDate,
                interviewProcess: newData?.interviewProcess,
                criteria: newData?.criteria,
                profileLink: newData?.profileLink,
                hiringManager: newData?.hiringManager,
    ...
    const Approve = async (e) => {
        ...
        const res = await HttpClient.requestData("approve-job-post/" + location?.state?.data?._id, "POST", data)
        if (res && res.status) {
                                ...
                                <div className=''>
                                    <form>
                                        <div className='fulljobdiv'>
                                            ...
                                            <div className='jobdtlsdiv'>
                                                <p className='jibtxt' style={{fontSize:"15px"}}>Department/Team <span class="required">*</span></p>
                                                <select
                                                    class="hiringslct"
